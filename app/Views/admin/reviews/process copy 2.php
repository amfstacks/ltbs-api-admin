...
<div class="fixed bottom-0 left-0 lg:left-64 right-0 bg-white border-t border-gray-200 shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.1)] z-40 p-4" 
     x-data="audioPlayer('<?= $highUrl ?>', '<?= $lowUrl ?>')">
    
    <audio id="review-audio" :src="currentSrc" @timeupdate="updateProgress()" @loadedmetadata="setDuration()" preload="metadata"></audio>
    ...
    document.addEventListener('alpine:init', () => {
        ...
        Alpine.data('audioPlayer', (highUrl, lowUrl) => ({
            ...
            toggleQuality() {
                ...
                // Wait for Vue/Alpine to update the DOM src, then restore playback state
                this.$nextTick(() => {
                    this.audio.load();
        ...
        Alpine.data('noteEngine', () => ({
            ...
            captureTime() {
                ...
                if(audioEl) {
                    ...
                    this.rawTime = Math.floor(audioEl.currentTime); // Save pure seconds for the database!
                }
            },
            formatTime(seconds) {
                if (!seconds || isNaN(seconds)) return "00:00";
